...
def export_dxf_10mm_a4portrait(dxf_path, output_pdf_path):
    ...
    try:
        ...
        sh = getattr(doc, "_sheet_h", sw * (277.0 / 190.0))
    ...
    print(f"[3/4] A4 Portrait: {PAPER_W_MM}x{PAPER_H_MM} mm  |  Printable: {printable_w_mm}x{printable_h_mm} mm  (10mm margins all)")
    ...
    temp_pdf = PDF_DIR / f"_temp_PORTRAIT_10mm_{dxf_path.stem}.pdf"
    ...
    print(f"    MediaBox: {PAPER_W_MM}x{PAPER_H_MM} mm A4 Portrait  |  Margins: {MARGIN_MM}mm all sides")
    return True


if __name__ == "__main__":
    print("=" * 72)
    print("RE-EXPORT DXF -> A4 PORTRAIT PDF  (10mm MARGINS ALL AROUND)")
    print("=" * 72)

    jobs = [
        (
            DXF_DIR / "BA-Refined-Ground-Floor-Plan.dxf",
            PDF_DIR / "BA-Refined-Ground-Floor-Plan-10mmMargin-A4Portrait.pdf",
            "GROUND FLOOR",
        ),
        (
            DXF_DIR / "BA-Refined-First-Floor-Plan.dxf",
            PDF_DIR / "BA-Refined-First-Floor-Plan-10mmMargin-A4Portrait.pdf",
            "FIRST FLOOR",
        ),
    ]
    ...
    for dxf, pdf, label in jobs:
        print(f"\n>>>>>  {label}  <<<<<")
        if not dxf.exists():
            ok = export_dxf_10mm_a4portrait(dxf, pdf)
            if not ok:
                all_ok = False
        else:
    ...
    print("\n" + "=" * 72)
    if all_ok:
        print("SUCCESS: Both PDFs exported with 10mm margins on A4 Portrait.")
    else:
