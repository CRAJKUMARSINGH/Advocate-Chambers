"""
Banswara Advocate Chambers - Project specific Implementation
Uses TraeCAD Engine for parametric CAD generation.
"""
import os
import math
...
def draw_sheet_01_site_plan():
    ...
    text_msp(msp, "PLOT 1 BOUNDARY (100'-0\" x 100'-0\" = 10,000 SQ FT)", ox + p1_w / 2.0, oy + p1_d + 2.5 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")

    gap = 8.0 * FT
    p2_ox = ox + p1_w + gap
    p2_w, p2_d = 65.0 * FT, 45.0 * FT
    rect(msp, p2_ox, oy + 27.0 * FT, p2_ox + p2_w, oy + 27.0 * FT + p2_d, layer="A-SITE", lw=45)
    text_msp(msp, "PLOT 2 BOUNDARY (70'-0\" x 50'-0\" = 3,500 SQ FT)", p2_ox + p2_w / 2.0, oy + 72.0 * FT + 2.5 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "PLOT 1 ADVOCATE COMPLEX\nBUILDABLE FOOTPRINT: 90'-0\" x 90'-0\" (8,100 SQ FT)\n115 BAYS (OPT 1) / 120 BAYS (OPT 2 G+1)", ox + 50.0 * FT, oy + 36.0 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "PLOT 2 ANNEX\nFOOTPRINT: 60'-0\" x 40'-0\" (2,400 SQ FT)\n35 BAYS + AMENITIES (OPT 1) / 30 BAYS (OPT 2)", p2_ox + 32.5 * FT, oy + 50.0 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")

    rect(msp, ox + p1_w, oy + 45.0 * FT, p2_ox, oy + 55.0 * FT, layer="A-ROOF", lw=25)
    text_msp(msp, "10' COVERED WALKWAY", ox + p1_w + gap / 2.0, oy + 50.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL", rot=90.0)

    plaza_y1 = oy - 14.0 * FT
    plaza_y2 = oy
    rect(msp, ox, plaza_y1, p2_ox + p2_w, plaza_y2, layer="A-SITE", lw=30)
    text_msp(msp, "20 ft LANDSCAPED PLAZA - PEDESTRIAN ENTRY FACING DISTRICT COURT MAIN ENTRANCE (SOUTH)", (ox + p2_ox + p2_w) / 2.0, oy - 7.0 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")
    ...
    for tx in [ox + 10.0 * FT, ox + 35.0 * FT, ox + 65.0 * FT, ox + 90.0 * FT, p2_ox + 15.0 * FT, p2_ox + 45.0 * FT]:
        ...
        text_msp(msp, "TREE", tx, oy - 7.0 * FT, h=TX_XXL * FT, layer="A-BAY")

    rect(msp, ox + 10.0 * FT, oy + p1_d - 4.0 * FT, ox + 60.0 * FT, oy + p1_d, layer="A-FURN")
    text_msp(msp, "25 TWO-WHEELER SLOTS (P1)", ox + 35.0 * FT, oy + p1_d - 2.0 * FT, h=TX_TITLE * FT, layer="A-FURN")

    rect(msp, p2_ox + 10.0 * FT, oy + 68.0 * FT, p2_ox + 50.0 * FT, oy + 72.0 * FT, layer="A-FURN")
    text_msp(msp, "15 TWO-WHEELER SLOTS (P2)", p2_ox + 30.0 * FT, oy + 70.0 * FT, h=TX_TITLE * FT, layer="A-FURN")

    rect(msp, ox, oy + 15.0 * FT, ox + 4.0 * FT, oy + 60.0 * FT, layer="A-FURN")
    text_msp(msp, "50 BICYCLE SLOTS", ox + 2.0 * FT, oy + 37.5 * FT, h=TX_TITLE * FT, layer="A-FURN", rot=90.0)
...
def draw_sheet_02_o1_p1_gf():
    ...
    msp.add_line((ox + WB / 2.0 + 5.0 * FT, oy), (ox + WB / 2.0 + 5.0 * FT, oy + 1.0 * FT), dxfattribs={"layer": "A-DOOR", "lineweight": 40})
    text_msp(msp, "10 ft MAIN DOUBLE-LEAF ENTRY (SOUTH - COURT FACING)", ox + WB / 2.0, oy + 2.0 * FT, h=TX_SUPER * FT, layer="A-DOOR")

    entry_y_top = oy + 8.0 * FT
    rect(msp, ox + 2.0 * FT, oy + 1.0 * FT, ox + 18.0 * FT, entry_y_top, layer="A-FURN")
    text_msp(msp, "RECEPTION 16'x8'\n(Low Counter - RPwD Access)", ox + 10.0 * FT, oy + 4.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")

    rect(msp, ox + 45.0 * FT, oy + 1.0 * FT, ox + 88.0 * FT, entry_y_top, layer="A-FURN")
    text_msp(msp, "LITIGANT WAITING HALL 43'x8'\n(80 FIXED CUSHIONED SEATS + 100 FOLDING SEATS + 3 NOTICE BOARDS)", ox + 66.5 * FT, oy + 4.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")

    corr_y_bot = oy + 34.0 * FT
    corr_y_top = oy + 40.0 * FT
    rect(msp, ox + 1.0 * FT, corr_y_bot, ox + WB - 1.0 * FT, corr_y_top, layer="A-WALL", lw=25)
    text_msp(msp, "====== 6 ft MAIN CENTRAL CORRIDOR (EAST-WEST) ======", ox + WB / 2.0, (corr_y_bot + corr_y_top) / 2.0, h=TX_SUPER * FT, layer="A-TEXT-TTL")

    cx1 = ox + WB - 22.0 * FT
    cy1 = oy + 9.5 * FT
    cx2 = ox + WB - 2.0 * FT
    cy2 = cy1 + 9.0 * FT
    # Removed MINI COURTYARD 20'x10' as requested
    ...
    text_msp(msp, "ROW A (BAYS 1-15: ADVOCATE NORTH, 2 LITIGANTS SOUTH)", start_x + n_cols * bw / 2.0, row_a_y + bd + 0.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    text_msp(msp, "4' AISLE 1", start_x + n_cols * bw / 2.0, row_a_y - 1.6 * FT, h=TX_TITLE * FT, layer="A-TEXT")
    text_msp(msp, "ROW B (BAYS 16-30: ADVOCATE SOUTH, 2 LITIGANTS NORTH)", start_x + n_cols * bw / 2.0, row_b_y - 0.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "ROW C (BAYS 31-45)", start_x + n_cols * bw / 2.0, row_c_y + bd + 0.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    text_msp(msp, "4' AISLE 2", start_x + n_cols * bw / 2.0, row_c_y - 1.6 * FT, h=TX_TITLE * FT, layer="A-TEXT")
    text_msp(msp, "ROW D (BAYS 46-60)", start_x + n_cols * bw / 2.0, row_d_y - 0.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "ROW E (BAYS 61-75)", start_x + n_cols * bw / 2.0, row_e_y + bd + 0.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    text_msp(msp, "4' AISLE 3", start_x + n_cols * bw / 2.0, row_e_y - 1.6 * FT, h=TX_TITLE * FT, layer="A-TEXT")
    text_msp(msp, "ROW F (BAYS 76-90)", start_x + n_cols * bw / 2.0, row_f_y - 0.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "ROW G (BAYS 91-105)", start_x + n_cols * bw / 2.0, row_g_y + bd + 0.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    text_msp(msp, "4' AISLE 4", start_x + n_cols * bw / 2.0, row_g_y - 1.6 * FT, h=TX_TITLE * FT, layer="A-TEXT")
    text_msp(msp, "ROW H (BAYS 106-115)", start_x + 10 * bw / 2.0, row_h_y - 0.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "RO PLANT (2x20 L/H)\n+ WATER COOLER", ro_x + 4.0 * FT, lock_y + 0.75 * FT, h=TX_XXL * FT, layer="A-FURN")

    rect(msp, ox + WB - 12.0 * FT, oy + HB - 2.0 * FT, ox + WB - 2.0 * FT, oy + HB, layer="A-DOOR", lw=40)
    text_msp(msp, "10 ft FIRE EXIT N", ox + WB - 7.0 * FT, oy + HB - 1.0 * FT, h=TX_TITLE * FT, layer="A-DOOR")
...
def draw_sheet_03_o1_p2_gf():
    ...
    text_msp(msp, "5 ft N-S CENTRAL AISLE", (centr_x1 + centr_x2) / 2.0, oy + 32.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL", rot=90.0)
    ...
    text_msp(msp, "ROW J (BAYS 116-125: ADVOCATE NORTH, 2 LITIGANTS SOUTH)", ox + WB / 2.0, row_j_y + bd + 0.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    text_msp(msp, "4' AISLE 5", ox + WB / 2.0, row_j_y - 2.2 * FT, h=TX_TITLE * FT, layer="A-TEXT")
    text_msp(msp, "ROW K (BAYS 126-135: ADVOCATE SOUTH, 2 LITIGANTS NORTH)", ox + WB / 2.0, row_k_y - 0.6 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "ROW L (BAYS 136-145)", ox + WB / 2.0, row_l_y + bd + 0.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    text_msp(msp, "4' AISLE 6", ox + WB / 2.0, row_l_y - 2.2 * FT, h=TX_TITLE * FT, layer="A-TEXT")
    text_msp(msp, "ROW M (BAYS 146-150)", sx + 15.0 * FT, row_m_y - 0.6 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")

    rect(msp, dx, row_m_y, dx + 30.0 * FT, row_m_y + bd, layer="A-BAY", lw=25)
    text_msp(msp, "FLEX SLOTS 151-155\n(VISITING / AD-HOC ADVOCATES)", dx + 15.0 * FT, row_m_y + bd / 2.0, h=TX_TITLE * FT, layer="A-BAY")
    ...
    text_msp(msp, "JR. ADVOCATE LOUNGE 22'x10'\n15 CHAIRS + 2 TABLES + RO WATER", ox + 13.0 * FT, oy + 6.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")

    rect(msp, ox + 26.0 * FT, oy + 1.0 * FT, ox + WB - 2.0 * FT, oy + 11.0 * FT, layer="A-FURN")
    text_msp(msp, "AMENITIES BLOCK 50'x10'\n3 GENTS WC + 2 URINALS | 3 LADIES WC | 1 RPwD ACCESSIBLE WC | 6 WASHBASINS + CLEAN STORE", ox + 53.0 * FT, oy + 6.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")

    msp.add_line((ox + WB / 2.0 - 4.0 * FT, oy), (ox + WB / 2.0 - 4.0 * FT, oy + 1.0 * FT), dxfattribs={"layer": "A-DOOR", "lineweight": 35})
    msp.add_line((ox + WB / 2.0 + 4.0 * FT, oy), (ox + WB / 2.0 + 4.0 * FT, oy + 1.0 * FT), dxfattribs={"layer": "A-DOOR", "lineweight": 35})
    text_msp(msp, "6 ft SOUTH ENTRY -> WALKWAY TO PLOT 1", ox + WB / 2.0, oy + 1.5 * FT, h=TX_TITLE * FT, layer="A-DOOR")

    rect(msp, ox + WB - 8.0 * FT, oy + HB - 2.0 * FT, ox + WB - 2.0 * FT, oy + HB, layer="A-DOOR", lw=35)
    text_msp(msp, "6 ft FIRE EXIT N", ox + WB - 5.0 * FT, oy + HB - 1.0 * FT, h=TX_TITLE * FT, layer="A-DOOR")
...
def draw_sheet_04_o1_combined_campus():
    ...
    text_msp(msp, "PLOT 1 - 115 ADVOCATE BAYS\n+ 150 ADVOCATE LOCKERS + WAITING HALL", p1_bx + 36.0 * FT, p1_by + 32.0 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "PLOT 2 - 35 ADVOCATE BAYS\n+ 40 LITIGANT LOCKERS\n+ AMENITIES + LOUNGE", p2_bx + 21.0 * FT, p2_by + 18.0 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")

    rect(msp, ox + 80.0 * FT, oy + 45.0 * FT, p2_ox, oy + 53.0 * FT, layer="A-ROOF", lw=25)
    text_msp(msp, "10 ft COVERED WALKWAY", ox + 84.0 * FT, oy + 49.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL", rot=90.0)

    rect(msp, ox, oy - 12.0 * FT, p2_ox + 50.0 * FT, oy, layer="A-SITE", lw=30)
    text_msp(msp, "20 ft LANDSCAPED PLAZA - PEDESTRIAN ACCESS FROM DISTRICT COURT", (ox + p2_ox + 50.0 * FT) / 2.0, oy - 6.0 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")

    text_msp(msp, ">> ADVOCATE & LITIGANT ENTRY CIRCULATION >>", p1_bx + 36.0 * FT, oy - 2.5 * FT, h=TX_TITLE * FT, layer="A-ACC")
...
def draw_sheet_05_o1_sections():
    ...
    text_msp(msp, "GROUND LEVEL (0'-0\")", ox - 6.0 * FT, oy, h=TX_TITLE * FT, layer="A-TEXT", align=TextEntityAlignment.RIGHT)

    rect(msp, ox, oy, ox + W, oy + plinth, layer="A-WALL", lw=40)
    fill_rect(msp, ox, oy, ox + W, oy + plinth, hatch="ANSI31", layer="A-HATCH")
    text_msp(msp, "PLINTH LEVEL (+1'-6\")", ox - 6.0 * FT, oy + plinth, h=TX_TITLE * FT, layer="A-TEXT", align=TextEntityAlignment.RIGHT)
    ...
    text_msp(msp, "ROOF SLAB LEVEL (+13'-6\") [125mm RCC M20 + 1 ft BRICKBAT COBA WATERPROOFING]", ox + W / 2.0, roof_bot + 0.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "PARAPET TOP (+16'-6\")", ox - 6.0 * FT, roof_top + parapet, h=TX_TITLE * FT, layer="A-TEXT-TTL", align=TextEntityAlignment.RIGHT)

    for jx in [ox + 16.0 * FT, ox + 32.0 * FT, ox + 68.0 * FT, ox + 84.0 * FT]:
        rect(msp, jx, wall_bot, jx + 0.4 * FT, wall_bot + 3.5 * FT, layer="A-JALI", lw=25)
        text_msp(msp, "3.5' JALI", jx + 0.2 * FT, wall_bot + 4.2 * FT, h=TX_XXL * FT, layer="A-JALI")

    text_msp(msp, "6 ft CENTRAL CORRIDOR (CLEAR FLOOR)", ox + 50.0 * FT, wall_bot + 2.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    for fx in [ox + 12.0 * FT, ox + 28.0 * FT, ox + 50.0 * FT, ox + 72.0 * FT, ox + 88.0 * FT]:
        ...
        text_msp(msp, "FAN", fx, roof_bot - 2.0 * FT, h=TX_XXL * FT, layer="A-FURN")
...
def draw_sheet_06_o1_elevations():
    ...
    text_msp(msp, "6\" RED SANDSTONE EMULSION PLINTH BAND", ox + W / 2.0, oy + 0.75 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "10 ft MAIN DOUBLE-LEAF ENTRY", ox + W / 2.0, oy + 5.0 * FT, h=TX_TITLE * FT, layer="A-DOOR")
    ...
    for wx in win_xs:
        ...
        text_msp(msp, "4'x4'", wx + 2.0 * FT, oy + 6.0 * FT, h=TX_TITLE * FT, layer="A-WINDOW")

    for wx in win_xs:
        rect(msp, wx, oy + 9.5 * FT, wx + 4.0 * FT, oy + 11.5 * FT, layer="A-JALI", lw=25)
        text_msp(msp, "JALI", wx + 2.0 * FT, oy + 10.5 * FT, h=TX_XXL * FT, layer="A-JALI")

    rect(msp, ox + 25.0 * FT, oy + 13.8 * FT, ox + 75.0 * FT, oy + 15.8 * FT, layer="A-TEXT-TTL", lw=25)
    text_msp(msp, "ADVOCATE SITOUT - BANSWARA DISTRICT COURT", ox + W / 2.0, oy + 14.8 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")
    text_msp(msp, "SOUTH (FRONT) ELEVATION - OPTION 1", ox + W / 2.0, oy - 2.5 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")
    ...
    for ex in [ox + 12.0 * FT, ox + W - 16.0 * FT]:
        ...
        text_msp(msp, "FIRE EXIT", ex + 2.0 * FT, oy2 + 5.0 * FT, h=TX_TITLE * FT, layer="A-DOOR")
    ...
    for wx in [ox + 24.0 * FT, ox + 36.0 * FT, ox + 48.0 * FT, ox + 60.0 * FT, ox + 72.0 * FT]:
        ...
        text_msp(msp, "4'x4'", wx + 2.0 * FT, oy2 + 6.0 * FT, h=TX_TITLE * FT, layer="A-WINDOW")

    text_msp(msp, "NORTH (REAR) ELEVATION - OPTION 1", ox + W / 2.0, oy2 - 2.5 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")
...
def draw_sheet_07_o2_p1_gf():
    ...
    text_msp(msp, "24'x24' OPEN-TO-SKY COURTYARD\nSTEPWELL FOUNTAIN + 4 TREES + 6 BENCHES\n(DOUBLE HEIGHT 23' CLEAR VOID)",
             (cx1 + cx2) / 2.0, (cy1 + cy2) / 2.0, h=TX_SUPER * FT, layer="A-TEXT-TTL")

    rect(msp, cx1 - 5.0 * FT, cy1 - 5.0 * FT, cx2 + 5.0 * FT, cy2 + 5.0 * FT, layer="A-WALL", lw=25)
    text_msp(msp, "6 ft RING CORRIDOR", (cx1 + cx2) / 2.0, cy1 - 2.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "2x LIFTS (13P MRL)\n+ 2x STAIRS (5' WIDE)", st_x + 8.0 * FT, oy + HB - 6.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "WEST CLUSTER: 28 OPEN BAYS (W1-W28)", ox + 14.0 * FT, oy + 40.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL", rot=90.0)
    ...
    text_msp(msp, "EAST CLUSTER: 24 OPEN BAYS (E1-E24)", ox + WB - 11.0 * FT, oy + 24.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL", rot=90.0)

    rect(msp, ox + 3.0 * FT, oy + 2.0 * FT, ox + 20.0 * FT, oy + 9.0 * FT, layer="A-FURN")
    text_msp(msp, "RECEPTION 17'x8'\n(2 Counters + Braille Desk)", ox + 11.5 * FT, oy + 5.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")

    rect(msp, ox + 45.0 * FT, oy + 2.0 * FT, ox + 88.0 * FT, oy + 9.0 * FT, layer="A-FURN")
    text_msp(msp, "LITIGANT WAITING HALL 43'x8' (75 CUSHIONED + 50 FOLDING SEATS)", ox + 66.5 * FT, oy + 5.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")

    msp.add_line((ox + WB / 2.0 - 5.0 * FT, oy), (ox + WB / 2.0 - 5.0 * FT, oy + 1.5 * FT), dxfattribs={"layer": "A-DOOR", "lineweight": 40})
    msp.add_line((ox + WB / 2.0 + 5.0 * FT, oy), (ox + WB / 2.0 + 5.0 * FT, oy + 1.5 * FT), dxfattribs={"layer": "A-DOOR", "lineweight": 40})
    text_msp(msp, "10 ft MAIN CARVED STONE ENTRY ARCH", ox + WB / 2.0, oy + 2.0 * FT, h=TX_TITLE * FT, layer="A-DOOR")
...
def draw_sheet_08_o2_p1_ff():
    ...
    text_msp(msp, "24'x24' COURTYARD VOID (OPEN TO SKY)\nOVERLOOKING GROUND FLOOR FOUNTAIN & TREES", (cx1 + cx2) / 2.0, (cy1 + cy2) / 2.0, h=TX_SUPER * FT, layer="A-TEXT-TTL")

    rect(msp, cx1 - 5.0 * FT, cy1 - 5.0 * FT, cx2 + 5.0 * FT, cy2 + 5.0 * FT, layer="A-WALL", lw=25)
    text_msp(msp, "6 ft RING CORRIDOR FLOOR", (cx1 + cx2) / 2.0, cy1 - 2.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "BAR ASSOCIATION ROOM 32'x10'\n(100 SEATS + BAR COUNTER)", ox + 19.0 * FT, oy + 7.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")

    rect(msp, ox + 40.0 * FT, oy + 2.0 * FT, ox + 88.0 * FT, oy + 12.0 * FT, layer="A-FURN")
    text_msp(msp, "LAW REFERENCE LIBRARY 48'x10'\n(1,000+ BOOKS + 4 COMPUTER STATIONS)", ox + 64.0 * FT, oy + 7.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")

    sb_y1 = oy + 32.0 * FT
    sb_y2 = sb_y1 + 10.0 * FT
    rect(msp, ox + WB, sb_y1, ox + WB + 15.0 * FT, sb_y2, layer="A-ROOF", lw=40)
    text_msp(msp, "10 ft COVERED SKY-BRIDGE -> TO PLOT 2 FIRST FLOOR", ox + WB + 7.5 * FT, (sb_y1 + sb_y2) / 2.0, h=TX_TITLE * FT, layer="A-ROOF")
...
def draw_sheet_09_o2_p1_2f():
    ...
    text_msp(msp, "BAR ASSOCIATION GRAND HALL 40'x30' (1,200 SQ FT)\n200-SEAT THEATRE CAPACITY | RAISED DAIS & PODIUM\n(PHASE 2 CONVERTIBLE TO 20 WORKSTATION BAYS)",
             ox + 23.0 * FT, oy + HB - 14.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "CENTRAL LAW LIBRARY 25'x30' (750 SQ FT)\n1,000+ BOOKS | 10 READING CARRELS | 4 PC TERMINALS\n(PHASE 2 CONVERTIBLE TO 16 BAYS + 4 CABINS)",
             ox + 59.5 * FT, oy + HB - 14.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "MULTI-PURPOSE HALL 30'x25' (750 SQ FT)\n40-SEAT CONFERENCE + PANTRY KITCHENETTE\n(PHASE 2 CONVERTIBLE TO 20 BAYS)",
             ox + 73.0 * FT, oy + 21.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "TERRACE ROOF GARDEN ~2,000 SQ FT (SOUTH OVERLOOK)\nPERGOLA COVER (15'x20') + 4 STONE BENCHES + NATIVE PLANTERS\n(PHASE 2: 4 GLASS PREMIUM CABINS POSSIBLE)",
             ox + 28.5 * FT, oy + 13.0 * FT, h=TX_TITLE * FT, layer="A-BAY")

    cx1 = ox + (WB - 24.0 * FT) / 2.0
    cy1 = oy + (HB - 24.0 * FT) / 2.0
    rect(msp, cx1, cy1, cx1 + 24.0 * FT, cy1 + 24.0 * FT, layer="A-JALI", lw=30)
    text_msp(msp, "24'x24' COURTYARD VOID", cx1 + 12.0 * FT, cy1 + 12.0 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")
...
def draw_sheet_10_o2_p2_gf():
    ...
    text_msp(msp, "COVERED TWO-WHEELER PARKING - 35 BIKES\n2 ROWS STEEL RACKS + 2 EV CHARGING POINTS (NORTH SERVICE ACCESS)",
             ox + WB / 2.0, oy + HB - 6.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "JR. LOUNGE 20'x5'\n20 SEATS + RO WATER", ox + 12.0 * FT, oy + 3.75 * FT, h=TX_TITLE * FT, layer="A-FURN")

    rect(msp, ox + 42.0 * FT, oy + 1.0 * FT, ox + WB - 2.0 * FT, oy + 6.5 * FT, layer="A-FURN")
    text_msp(msp, "AMENITIES: 4G+4L WCs | 4 URINALS | 2 RPwD ACCESSIBLE WCs | 6 BASINS", ox + 58.0 * FT, oy + 3.75 * FT, h=TX_TITLE * FT, layer="A-FURN")

    rect(msp, ox + 24.0 * FT, oy + 1.0 * FT, ox + 40.0 * FT, oy + 6.5 * FT, layer="A-WALL", lw=30)
    text_msp(msp, "LIFT 10P + STAIR 5' W", ox + 32.0 * FT, oy + 3.75 * FT, h=TX_TITLE * FT, layer="A-WALL")
...
def draw_sheet_11_o2_p2_ff():
    ...
    text_msp(msp, "CONFERENCE ROOM 30'x17'\n40-SEAT AUDITORIUM | 100\" SCREEN + PA", ox + 17.0 * FT, oy + 9.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")

    rect(msp, ox + 45.0 * FT, oy + 1.0 * FT, ox + WB - 2.0 * FT, oy + 18.0 * FT, layer="A-FURN", lw=30)
    text_msp(msp, "BAR CAFETERIA & DINING 33'x17'\n50 SEATS + KITCHENETTE & VENDING", ox + 61.0 * FT, oy + 9.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")

    rect(msp, ox - 15.0 * FT, oy + 18.0 * FT, ox, oy + 28.0 * FT, layer="A-ROOF", lw=40)
    text_msp(msp, "<- 10 ft SKY-BRIDGE TO PLOT 1 FF RING CORRIDOR", ox - 7.5 * FT, oy + 23.0 * FT, h=TX_TITLE * FT, layer="A-ROOF")
...
def draw_sheet_02_12_o2_p1_section():
    ...
    for sl_top, sl_name in [(lvl_ff_slab, "1F SLAB (+12'-0\")"), (lvl_2f_slab, "2F SLAB (+24'-0\")"), (lvl_roof_slab, "ROOF SLAB (+36'-0\")")]:
        ...
        text_msp(msp, sl_name, ox + W / 2.0, sl_top - 0.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "24'x24' OPEN-TO-SKY COURTYARD VOID\n(DOUBLE HEIGHT 23' CLEAR GF->2F)", (cx1 + cx2) / 2.0, (lvl_plinth + lvl_2f_slab) / 2.0, h=TX_SUPER * FT, layer="A-TEXT-TTL")

    msp.add_circle(((cx1 + cx2) / 2.0, lvl_plinth + 2.0 * FT), 2.5 * FT, dxfattribs={"layer": "A-ROOF"})
    text_msp(msp, "FOUNTAIN", (cx1 + cx2) / 2.0, lvl_plinth + 2.0 * FT, h=TX_TITLE * FT, layer="A-ROOF")

    sb_x1 = ox + W
    rect(msp, sb_x1, lvl_ff_slab - 0.5 * FT, sb_x1 + 12.0 * FT, lvl_ff_slab + 4.5 * FT, layer="A-ROOF", lw=35)
    text_msp(msp, "SKY-BRIDGE -> PLOT 2 FF (@ 12 ft)", sb_x1 + 6.0 * FT, lvl_ff_slab + 2.0 * FT, h=TX_TITLE * FT, layer="A-ROOF")
    ...
    for yval, label in [(oy, "GL: 0'-0\""), (lvl_plinth, "PLINTH: +1'-6\""), (lvl_ff_slab, "1F: +12'-0\""), (lvl_2f_slab, "2F: +24'-0\""), (lvl_roof_slab, "ROOF: +36'-0\""), (lvl_parapet, "PARAPET: +39'-0\"")]:
        ...
        text_msp(msp, label, lmx + 1.5 * FT, yval, h=TX_TITLE * FT, layer="A-TEXT-TTL", align=TextEntityAlignment.LEFT)
...
def draw_sheet_13_o2_p2_section():
    ...
    text_msp(msp, "10 ft SKY-BRIDGE -> CONNECTS TO PLOT 1 FF (@ 12'-0\")", (sb_x1 + ox) / 2.0, lvl_ff_slab + 2.0 * FT, h=TX_TITLE * FT, layer="A-ROOF")

    text_msp(msp, "GF: 15 BAYS + 35 BIKE PARKING + AMENITIES", ox + W / 2.0, lvl_plinth + 5.5 * FT, h=TX_SUPER * FT, layer="A-TEXT")
    text_msp(msp, "FF: 15 BAYS + 40-SEAT CONF. + 50-SEAT DINING", ox + W / 2.0, lvl_ff_slab + 5.5 * FT, h=TX_SUPER * FT, layer="A-TEXT")
...
def draw_sheet_14_heritage_and_details():
    ...
    text_msp(msp, "2 ft PINK BANSWARA SANDSTONE PLINTH", ox + W / 2.0, oy + 1.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    for i in range(9):
        ...
        text_msp(msp, f"JHAROKHA {i + 1}\n(CARVED STONE)", jx + 3.5 * FT, oy + 12.0 * FT, h=TX_TITLE * FT, layer="A-WINDOW")

    rect(msp, ox - 2.0 * FT, oy + 30.0 * FT, ox + W + 2.0 * FT, oy + 31.5 * FT, layer="A-ROOF", lw=40)
    text_msp(msp, "4 ft DEEP FULL-WIDTH CANTILEVERED STONE CHAJJAS (SUN & MONSOON PROTECTION)", ox + W / 2.0, oy + 30.75 * FT, h=TX_TITLE * FT, layer="A-ROOF")

    rect(msp, ox + W / 2.0 - 6.0 * FT, oy + 2.0 * FT, ox + W / 2.0 + 6.0 * FT, oy + 14.0 * FT, layer="A-DOOR", lw=45)
    text_msp(msp, "MAIN ENTRY TORAN ARCH\n(12 ft HIGH PYLONS)", ox + W / 2.0, oy + 8.0 * FT, h=TX_TITLE * FT, layer="A-DOOR")

    text_msp(msp, "OPTION 2 - SOUTH FRONT HERITAGE ELEVATION (RAJASTHANI HAVELI ARCHITECTURE)", ox + W / 2.0, oy + H + 2.0 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")

    # Section 2: Detailed Bay Typologies with Opposite-Side Seating
    bx_typ = 14.0 * FT
    by_typ = 11.0 * FT
    text_msp(msp, "BAY TYPOLOGY A: COMPACT (5'x6.5')", bx_typ + 4.0 * FT, by_typ + 15.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    bay_compact(msp, bx_typ, by_typ + 4.0 * FT, 6.0 * FT, 7.5 * FT, "TYP-A", orientation="S")
    text_msp(msp, "(Advocate Inner Side | 2 Litigants Opposite Side)", bx_typ + 4.0 * FT, by_typ + 2.5 * FT, h=TX_XXL * FT, layer="A-TEXT")

    bx_prem = bx_typ + 15.0 * FT
    text_msp(msp, "BAY TYPOLOGY B: PREMIUM CABIN (8'x10')", bx_prem + 4.0 * FT, by_typ + 15.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    bay_premium(msp, bx_prem, by_typ + 3.0 * FT, 8.0 * FT, 9.5 * FT, "TYP-B")
    text_msp(msp, "(Senior Advocate Inner Side | 3 Clients Opposite Side)", bx_prem + 4.0 * FT, by_typ + 1.8 * FT, h=TX_XXL * FT, layer="A-TEXT")
    ...
    text_msp(msp, "CAMPUS LOCKER SYSTEM SPECIFICATIONS & BUDGET ESTIMATE (190 TOTAL)", sch_x + sch_w / 2.0, sch_y + sch_h - 1.2 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    for i, sp in enumerate(locker_specs):
        text_msp(msp, sp, sch_x + 1.0 * FT, sch_y + sch_h - 3.2 * FT - i * 1.8 * FT, h=TX_XXL * FT, layer="A-TEXT", align=TextEntityAlignment.LEFT)
...
def draw_sheet_15_type_a_sitout():
    ...
    text_msp(msp, "3.5 ft HIGH BRICK JALI SCREEN (REAR)", px + pw / 2.0, py + pd - 0.9 * FT, h=TX_TITLE * FT, layer="A-JALI")

    rect(msp, px, py, px + 1.8 * FT, py + pd, layer="A-JALI", lw=30)
    fill_rect(msp, px, py, px + 1.8 * FT, py + pd, hatch="ANSI31", layer="A-HATCH")
    text_msp(msp, "3.5 ft JALI", px + 0.9 * FT, py + pd / 2.0, h=TX_TITLE * FT, layer="A-JALI", rot=90.0)
    ...
    text_msp(msp, "MODULAR CONSULTATION DESK\n4'-0\" x 1'-6\" (1200 x 450 mm)\n18mm PRE-LAM BWR PLY + 2mm PVC EDGE",
             (dx1 + dx2) / 2.0, (dy1 + dy2) / 2.0, h=TX_TITLE * FT, layer="A-FURN")

    rect(msp, dx2 - 7.0 * FT, dy1, dx2, dy2, layer="A-FURN", lw=20)
    text_msp(msp, "3-DRAWER\nPEDESTAL", dx2 - 3.5 * FT, (dy1 + dy2) / 2.0, h=TX_XXL * FT, layer="A-FURN")
    ...
    text_msp(msp, "ADVOCATE CHAIR\n(ERGONOMIC MESH MID-BACK SWIVEL)", adv_cx, adv_cy, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    text_msp(msp, "<- 2'-2\" ADVOCATE ZONE ->", adv_cx, py + pd - 3.5 * FT, h=TX_TITLE * FT, layer="A-DIM")
    ...
    for lcx, lbl in [(lit1_cx, "LITIGANT 1"), (lit2_cx, "LITIGANT 2")]:
        ...
        text_msp(msp, f"{lbl}\nVISITOR CHAIR", lcx, lit_cy, h=TX_TITLE * FT, layer="A-FURN")
    text_msp(msp, "<- 2'-1\" LITIGANT VISITOR ZONE (24\" C/C SPACING) ->", (lit1_cx + lit2_cx) / 2.0, py + 2.5 * FT, h=TX_TITLE * FT, layer="A-DIM")

    # Aisle Access
    rect(msp, px, py - 4.5 * FT, px + pw, py, layer="A-ACC", lw=25)
    text_msp(msp, "=== 4'-0\" CLEAR PUBLIC CIRCULATION AISLE ===", px + pw / 2.0, py - 2.25 * FT, h=TX_TITLE * FT, layer="A-ACC")
    ...
    text_msp(msp, "PLAN: TYPE A COMPACT SITOUT (1 ADVOCATE vs 2 OPPOSITE LITIGANTS)", px + pw / 2.0, py + pd + 6.0 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "PLINTH LEVEL (FL: 0'-0\")", sx + 10.0 * FT, sy - 1.5 * FT, h=TX_TITLE * FT, layer="A-TEXT")
    ...
    text_msp(msp, "3'-6\" BRICK JALI SCREEN", sx + 3.5 * FT, sy + 23.0 * FT, h=TX_TITLE * FT, layer="A-JALI")
    ...
    text_msp(msp, "ADVOCATE\nSEATED (INNER)", adv_seat_x, sy + 16.0 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "DESK 2'-6\" HT\n(MODESTY PANEL)", desk_sx + desk_sw / 2.0, sy + 7.5 * FT, h=TX_TITLE * FT, layer="A-FURN")
    ...
    text_msp(msp, "2x LITIGANTS\nSEATED (VISITOR)", lit_seat_x, sy + 16.0 * FT, h=TX_TITLE * FT, layer="A-FURN")

    rect(msp, sx + 52.0 * FT, sy, sx + sw_v, sy + 2.0 * FT, layer="A-ACC", lw=20)
    text_msp(msp, "4'-0\" AISLE", sx + 58.5 * FT, sy + 5.0 * FT, h=TX_TITLE * FT, layer="A-ACC")

    msp.add_line((sx, sy + 38.0 * FT), (sx + sw_v, sy + 38.0 * FT), dxfattribs={"layer": "A-ROOF", "lineweight": 30})
    text_msp(msp, "CEILING LEVEL (+11'-0\" CLEAR)", sx + 12.0 * FT, sy + 39.5 * FT, h=TX_TITLE * FT, layer="A-ROOF")
    text_msp(msp, "LED BATTEN 20W + BLDC CEILING FAN", sx + 40.0 * FT, sy + 35.5 * FT, h=TX_XXL * FT, layer="A-FURN")
    ...
    text_msp(msp, "SIDE ELEVATION / SECTION S-01: ERGONOMIC SITTING CLEARANCE", sx + sw_v / 2.0, sy + pd + 6.0 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "3'-6\" BRICK JALI SCREEN WITH SMOOTH CEMENT COPING", fx + fw / 2.0, fy + 22.2 * FT, h=TX_TITLE * FT, layer="A-JALI")
    ...
    text_msp(msp, "LOCKABLE\n3-DRAWER\nPEDESTAL", dfx2 - 4.0 * FT, (dfy1 + dfy2) / 2.0, h=TX_XXL * FT, layer="A-FURN")
    text_msp(msp, "4'-0\" MODULAR DESK FRONT (MODESTY PANEL)", (dfx1 + dfx2 - 8.0 * FT) / 2.0, (dfy1 + dfy2) / 2.0, h=TX_TITLE * FT, layer="A-FURN")

    msp.add_circle(((dfx1 + dfx2 - 8.0 * FT) / 2.0, fy + 17.5 * FT), 2.5 * FT, dxfattribs={"layer": "A-FURN", "lineweight": 20})
    text_msp(msp, "ADVOCATE CHAIR", (dfx1 + dfx2 - 8.0 * FT) / 2.0, fy + 17.5 * FT, h=TX_XXL * FT, layer="A-FURN")

    text_msp(msp, "FRONT ELEVATION E-01 (VIEW FROM VISITOR/AISLE SIDE)", fx + fw / 2.0, fy + fh + 1.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "TYPE A SITOUT - SPECIFICATIONS, ELECTRICAL & COST SCHEDULE", tx + tw / 2.0, ty + th - 1.2 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
...
def draw_sheet_16_type_b_cabin():
    ...
    text_msp(msp, "3'-0\"x7'-0\" FLUSH DOOR\n(VISION GLASS + MORTISE LOCK)", px + door_w / 2.0, py - 2.5 * FT, h=TX_TITLE * FT, layer="A-DOOR")
    ...
    text_msp(msp, "HIGH-BACK EXECUTIVE\nLEATHERETTE RECLINER", adv_cx, adv_cy, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    for i in range(3):
        ...
        text_msp(msp, f"CLIENT {i + 1}", cx, cy, h=TX_XXL * FT, layer="A-FURN")
    text_msp(msp, "<- 3x CLIENT VISITOR CHAIRS (ARMREST TYPE) ->", (dx1 + dx2) / 2.0, py + 4.5 * FT, h=TX_TITLE * FT, layer="A-DIM")
    ...
    text_msp(msp, "FULL-HEIGHT LAW BOOKSHELF & CASE ARCHIVE (6'-0\" x 1'-0\")",
             px + pw - 2.9 * FT, (py + 8.0 * FT + py + pd - 4.0 * FT) / 2.0, h=TX_TITLE * FT, layer="A-FURN", rot=90.0)
    ...
    text_msp(msp, "PLAN: TYPE B SENIOR ADVOCATE CABIN (8'-0\" x 10'-0\" = 80 SQ FT)", px + pw / 2.0, py + pd + 6.0 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "VISION GLASS\n12\"x36\"", (door_fx1 + door_fx2) / 2.0, fy + 23.0 * FT, h=TX_XXL * FT, layer="A-WINDOW")
    text_msp(msp, "BRASS NAMEPLATE\n& MORTISE LOCK", (door_fx1 + door_fx2) / 2.0, fy + 10.0 * FT, h=TX_XXL * FT, layer="A-DOOR")

    rect(msp, door_fx2 + 4.0 * FT, fy + 25.0 * FT, fx + fw - 6.0 * FT, fy + 33.0 * FT, layer="A-JALI", lw=25)
    text_msp(msp, "ACOUSTIC JALI LOUVERS (AIRFLOW + SOUND PRIVACY)", (door_fx2 + 4.0 * FT + fx + fw - 6.0 * FT) / 2.0, fy + 29.0 * FT, h=TX_XXL * FT, layer="A-JALI")

    rect(msp, door_fx2 + 4.0 * FT, fy + 4.0 * FT, fx + fw - 6.0 * FT, fy + 22.0 * FT, layer="A-WALL", lw=20)
    text_msp(msp, "ACOUSTIC FABRIC WALL PANELLING\n(WARM WOOD GRAIN TEXTURE)", (door_fx2 + 4.0 * FT + fx + fw - 6.0 * FT) / 2.0, fy + 13.0 * FT, h=TX_TITLE * FT, layer="A-WALL")

    arch_dim_v(msp, fx - 1.5 * FT, fy, fy + 35.0 * FT, "7'-0\" (2100mm) PARTITION HT", offset=-1.0 * FT)
    text_msp(msp, "FRONT ELEVATION E-02: 7 ft HIGH CABIN ENTRANCE FACADE", fx + fw / 2.0, fy + pd + 6.0 * FT, h=TX_SUPER * FT, layer="A-TEXT-TTL")
    ...
    text_msp(msp, "EXECUTIVE DESK 5'x2.5' (2'-6\" HT)", ix + 20.0 * FT, iy + 7.5 * FT, h=TX_TITLE * FT, layer="A-FURN")
    ...
    text_msp(msp, "LAW REPORTS & BINDERS\n(4-TIER BOOKSHELF)", ix + 45.5 * FT, iy + 11.0 * FT, h=TX_XXL * FT, layer="A-FURN")

    text_msp(msp, "INTERIOR ELEVATION S-02 (LOOKING TOWARD STORAGE)", ix + iw / 2.0, iy + ih + 1.5 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
    ...
    tw, th = 65.0 * FT, 25.0 * FT
    rect(msp, tx, ty, tx + tw, ty + th, layer="A-TTLB", lw=30)
    text_msp(msp, "TYPE B SENIOR CABIN - SPECIFICATIONS & COST SCHEDULE", tx + tw / 2.0, ty + th - 1.2 * FT, h=TX_TITLE * FT, layer="A-TEXT-TTL")
